...
class CleantalkValidate
{
    ...
    static public function apbct_spam_test($data): void
    {
        ...
        if (!$skip) {
            ...
            // Misc params
            $ct_request->js_on = self::apbct_js_test();
            $ct_request->submit_time = self::apbct_get_submit_time();
            ...
            $ct_request->post_info = $registration ? '' : json_encode(['comment_type' => 'feedback']);
    ...
    static private function apbct_get_sender_info($data)
    {
        return $sender_info = [
            ...
            'ct_options' => json_encode(['auth_key' => config('cleantalk.apikey'), 'response_lang' => 'en']),
     ...
     * return null|0|1;
     */
    static private function apbct_js_test()
    {
        if (isset($_COOKIE['apbct_checkjs'])) {
            if ($_COOKIE['apbct_checkjs'] == md5(config('cleantalk.apikey')))
                return 1;
            else
                return 0;
        } else {
     ...
     * @return null|int;
     */
    static private function apbct_get_submit_time()
    {
        $cookie_test_result = self::apbct_cookies_test();
     ...
     * @return null|int;
     */
    static private function apbct_get_ip()
    {
    ...
    * @return array
    */
    static private function apbct_get_fields_any($arr, $message = [], $email = null, $nickname = ['nick' => '', 'first' => '', 'last' => ''], $subject = null, $skip = false, $reg = false, $not_reg = false, $prev_key = '')
    {
        global $detected_cms;
                ...
                if (!is_array($value) && !is_object($value)) {
                    ...
                    foreach ($obfuscate_params as $needle) {
                        if (strpos($key, $needle) !== false) {
                            $value = self::apbct_obfuscate_param($value);
                            continue(2);
                        }
                    }
                    unset($needle);
                    ...
                    $message = ($temp['subject'] ? array_merge(['subject' => $temp['subject']], $temp['message']) : $temp['message']);
